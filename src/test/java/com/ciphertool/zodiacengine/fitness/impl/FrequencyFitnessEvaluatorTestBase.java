	...
	private static void populateSolutionLetterTotals() {
		...
		for (PlaintextSequence plaintext : knownSolution.getPlaintextCharacters()) {
			if (plaintext.getSequenceId() < numCharsToEvaluate) {
				...
				knownSolutionLetterTotals.put(plaintextChar, knownSolutionLetterTotals.get(plaintextChar) + 1);
			}
		}
		...
		log.info("Average word length known: " + ((double) numCharsToEvaluate / (double) (validWordCount)));
	}
}
