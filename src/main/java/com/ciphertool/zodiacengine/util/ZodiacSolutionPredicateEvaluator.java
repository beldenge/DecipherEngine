	...
	public int determineConfidenceLevel(Solution solution) {
		...
		for (List<Ciphertext> ciphertextIndices : ciphertextKey.values()) {
			...
			for (Ciphertext ciphertextIndice : ciphertextIndices) {
				...
				/*
				 *  The predicate used here just returns the Plaintext character that corresponds to the given Ciphertext character.
				 *  Using the Predicate to find the Plaintext Id is about 10 times less performant than using List.get() on the CiphertextId
				 */
				plaintext = (Plaintext) CollectionUtils.find(solution.getPlaintextCharacters(), new PlaintextPredicate(ciphertextIndice));
				...
				for (Ciphertext otherCiphertext: ciphertextIndices) {
					 ...
					 *  Using the Predicate to find the Plaintext Id is about 10 times less performant than using List.get() on the CiphertextId
					 */
					otherPlaintext = (Plaintext) CollectionUtils.find(solution.getPlaintextCharacters(), new PlaintextPredicate(otherCiphertext));
